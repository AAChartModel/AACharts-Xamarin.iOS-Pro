...
namespace AAChartsDotNet
{
    ...
    public class AALabels
    {
        ...
        public object autoRotationLimit;//当每个分类的宽度比该参数的值大很多（像素）时，轴标签将不会被自动旋转，而是以换行的形式展示轴标签。 当轴标签包含多个短词时换行展示轴标签可以使得轴标签有足够的空间，所以设置合理的自动旋转下限是非常有意义的。 默认是：80.
        public object distance;//只针对极地图有效，定义周标签与绘图区边缘的距离。 默认是：15.
        public object enabled;//是否显示坐标轴标签 默认是：true.
        public string format;//坐标轴格式化字符串。 默认是：{value}.
        public string formatter;//坐标轴格式化字符串。 默认是：{value}.
        public object padding;//轴标签的内间距，作用是保证轴标签之间有空隙。 默认是：5.
        public object rotation;//轴标签的旋转角度 默认是：0.
        public object staggerLines;//只针对水平轴有效，定义轴标签显示行数。
        public object step;//显示 n 的倍数标签，例如设置为 2 则表示标签间隔一个轴标签显示。默认情况下，为了避免轴标签被覆盖，该参数会根据情况自动计算。可以通过设置此参数为 1 来阻止自动计算。
        public AAStyle style;//轴标签的 CSS 样式
        public object x;//相对于坐标轴刻度线的水平偏移。 默认是：0.
        public object y;//相对于坐标轴刻度线的垂直平偏移。 默认是：null.
        public object useHTML;//HTML渲染
        ...
        public AALabels Enabled(bool prop)
        {
            enabled = prop;
            return this;
        }
        ...
        public AALabels StaggerLines(int prop)
        {
            staggerLines = prop;
            return this;
        }

        public AALabels Step(int prop)
        {
            step = prop;
            return this;
        }
        ...
        public AALabels UseHTML(bool prop)
        {
            useHTML = prop;
            return this;
        }
